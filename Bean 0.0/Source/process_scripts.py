...
def save_scripts(variable_list,variable_uses,scripts,tag_uses,quick_strings):
  ...
  for i_script in xrange(len(scripts)):
    ...
    if (not((script_name.startswith("game_")) or (script_name.startswith("multiplayer_")) or (script_name.startswith("wse_")))):
       if script_protection==1:
         script_name = signature[i_script%sign_length]                    # for Final Release
       else:  
         script_name = signature[i_script%sign_length]+"_"+str(i_script)   # for DEBUGING
    if (type(func[1]) == list_type):
      #file.write("%s -1\n"%(convert_to_identifier(func[0])))
      file.write("%s -1\n"%script_name)
      save_statement_block(file,convert_to_identifier(func[0]), 0,func[1], variable_list,variable_uses,tag_uses,quick_strings, "script_" + convert_to_identifier(func[0]) )
    else:
      #file.write("%s %f\n"%(convert_to_identifier(func[0]), func[1]))
      file.write("%s %f\n"%(script_name, func[1]))
      save_statement_block(file,convert_to_identifier(func[0]), 0,func[2], variable_list,variable_uses,tag_uses,quick_strings, "script_" + convert_to_identifier(func[0]) )
    #################################################################
    file.write("\n")
  file.close()
