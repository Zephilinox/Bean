...
def generate_presentation_load_script(_mod_options = mod_options):
		...
		if aModOption.GetType() == xgm_ov_line:
			...
			## WINDYPLAINS+ ## - Title Bar
				...
				opblock.Append([   
					...
					(overlay_set_position, reg1, pos1),
					(overlay_set_color, reg1, 1508608),
	...
	opblock.Append([                       
		...
		## Mouse-over Tips	
		#(str_store_string, s0, "@Mouse-over options for further information."),	##BEAN - Disable default message
		(create_text_overlay, mouse_over, s0, tf_double_space|tf_scrollable),
		(position_set_x, pos1, 650),
		(position_set_y, pos1, 150), #375 #400 ##BEAN - Increase tooltip size | Was 350, for every increase here, decrease below
		(overlay_set_position, mouse_over, pos1),
		(position_set_x, pos1, 800),
		(position_set_y, pos1, 800),
		(overlay_set_size, mouse_over, pos1),
		(position_set_x, pos1, 300),
		(position_set_y, pos1, 400), #175 #150	##BEAN - Increase tooltip size | Was 150, for every increase here, decrease above
		...
		(create_game_button_overlay, overlay_var, "@Faction Colors", tf_center_justify),
		(overlay_set_position, overlay_var, pos1),	
...
def generate_presentation_mouse_enter_leave_script(_mod_options = mod_options): ##Caba addition
	...
	opblock.Append([                       
			(else_try),
				#(overlay_set_text, mouse_over, "@Mouse-over options for further information."),	##BEAN - Disable default message
			(try_end),
		(try_end),
	])    
	cur_overlay_index += 1 #???? 
