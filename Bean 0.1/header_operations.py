################################################################################
#                                             header_operations expanded RC2.1 #
################################################################################
# TABLE OF CONTENTS
################################################################################
#
...
party_set_marshall                    = 1604  # (party_set_marshall, <party_id>, <value>),
                                              # Sets party as a marshall party or turns it back to normal party. Value is either 1 or 0. This affects party behavior, but exact effects are not known.
...
cur_item_set_tableau_material                    = 1981  # (cur_item_set_tableu_material, <tableau_material_id>, <instance_code>),
                                                         # Can only be used inside ti_on_init_item trigger in module_items.py. Assigns tableau to the item instance. Commonly used for heraldic armors and shields.
cur_scene_prop_set_tableau_material              = 1982  # (cur_scene_prop_set_tableau_material, <tableau_material_id>, <instance_code>),
                                                         # Can only be used inside ti_on_init_scene_prop trigger in module_scene_props.py. Assigns tableau to the scene prop instance. Commonly used for static banners.
cur_map_icon_set_tableau_material                = 1983  # (cur_map_icon_set_tableau_material, <tableau_material_id>, <instance_code>),
                                                         # Can only be used inside ti_on_init_map_icon trigger in module_map_icons.py. Assigns tableau to the icon prop instance. Commonly used for player/lord party banners.
...
cur_agent_set_banner_tableau_material            = 1986  # (cur_agent_set_banner_tableau_material, <tableau_material_id>)
                                                         # Can only be used inside ti_on_agent_spawn trigger in module_mission_templates. Assigns heraldry which will be used to render this agent on the scene.
...
cur_tableau_set_override_flags                   = 1997  # (cur_tableau_set_override_flags, <value>),
                                                         # When creating a troop image for current tableau, this operation will override current troop's inventory partially or completely.
...
cur_tableau_add_mesh_with_scale_and_vertex_color = 2000  # (cur_tableau_add_mesh_with_scale_and_vertex_color, <mesh_id>, <position>, <position>, <value_fixed_point>, <value>),
                                                         # Not documented. 4research. second position is x,y,z scale factors (with fixed point values). value fixed point is alpha. value is vertex color. use 0 for default values. scale and vertex_color has no default values.
...
str_encode_url                  = 2355  # (str_encode_url, <string_register>),
                                        # This operation will prepare an URL string to be used by the (send_message_to_url) operation.
...
scene_set_day_time                           = 1266  # (scene_set_day_time, <value>),
                                                     # Defines the time for the scene. Must be called within ti_before_mission_start trigger in module_mission_templates.py. Value should be in range 0..23.
...
mission_cam_set_mode                         = 2001  # (mission_cam_set_mode, <mission_cam_mode>, <duration-in-1/1000-seconds>, <value>) # 
                                                     # Not documented. when leaving manual mode, duration defines the animation time from the initial position to the new position. set as 0 for instant camera position update
mission_cam_set_screen_color                 = 2008  # (mission_cam_set_screen_color, <value>),
                                                     # Not documented. value is color together with alpha
...
scene_prop_set_hit_points                   = 1814  # (scene_prop_set_hit_points, <scene_prop_id>, <value>),
                                                    # Sets the number of hit points that the scene prop has (only makes sense for sokf_destructible scene props).
scene_prop_set_cur_hit_points               = 1820  # (scene_prop_set_cur_hit_points, <scene_prop_id>, <value>),
                                                    # Version 1.153+. UNTESTED. It is not clear how this is different from (scene_prop_set_hit_points).
...
prop_instance_enable_physics                = 1864  # (prop_instance_enable_physics, <scene_prop_id>, <value>),
                                                    # Enables (value = 1) or disables (value = 0) physics calculation (collision checks?) for the scene prop instance.
...
agent_get_player_id                      = 1724  # (agent_get_player_id, <destination>, <agent_id>),
                                                 # Retrieves the reference to player for specified agent.
...
agent_set_no_death_knock_down_only       = 1733  # (agent_set_no_death_knock_down_only, <agent_id>, <value>),
                                                 # Set the agent as immortal (value = 1) or regular (value = 0). Immortal agents behave like heroes, i.e. they are always knocked down and never killed.
...
agent_set_ranged_damage_modifier         = 2099  # (agent_set_ranged_damage_modifier, <agent_id>, <value>),
                                                 # Version 1.157+. Same as previous, but only affects ranged damage. Value is in percentage, 100 is default, 1000 is max possible value.
...
agent_set_no_dynamics                    = 1762  # (agent_set_no_dynamics, <agent_id>, <value>),
                                                 # Makes the agent stand on the spot (value = 1) or move normally (value = 0). When frozeon on the spot the agent can still turn around and fight if necessary. Used in Native for the wedding scene.
...
agent_ai_set_can_crouch                  = 2083  # (agent_ai_set_can_crouch, <agent_id>, <value>),
                                                 # Version 1.153+. Allows or forbits the agent to crouch. 0 to forbid, 1 to allow.
...
agent_set_attached_scene_prop            = 1757  # (agent_set_attached_scene_prop, <agent_id>, <scene_prop_id>)
                                                 # Attaches the specified prop instance to the agent.
...
agent_set_scripted_destination           = 1730  # (agent_set_scripted_destination, <agent_id>, <position>, [auto_set_z_to_ground_level]),
                                                 # Forces the agent to travel to specified position and stay there until new behavior is set or scripted mode cleared. Optional parameter determines whether the position Z coordinate will be automatically set to ground level (value = 1) or not (value = 0).
...
create_combo_label_overlay                        =  948  # (create_combo_label_overlay, <destination>),
                                                          # Creates a combo label overlay. Individual items can be added with (overlay_add_item) and currently selected item can be set with (overlay_set_val). Returns combo block's overlay_id.
create_combo_button_overlay                       =  916  # (create_combo_button_overlay, <destination>),
                                                          # Creates a combo button overlay. Individual items can be added with (overlay_add_item) and currently selected item can be set with (overlay_set_val). Returns combo block's overlay_id.
overlay_add_item                                  =  931  # (overlay_add_item, <overlay_id>, <string_id>),
                                                          # Adds an item to the listbox or combobox.
...
overlay_set_boundaries                            =  928  # (overlay_set_boundaries, <overlay_id>, <min_value>, <max_value>),
                                                          # Changes the boundaries for the overlays that have them.
...
overlay_set_additional_render_height              =  952  # (overlay_set_additional_render_height, <overlay_id>, <height_adder>),
                                                          # Version 1.153+. UNTESTED. Effects uncertain. 4research.
...
show_troop_details                                = 2388  # (show_troop_details, <troop_id>, <position>, <troop_price>)
                                                          # Version 1.153+. UNTESTED. Supposedly displays a popup with troop information at specified place.
