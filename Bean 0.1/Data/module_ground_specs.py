...
def write_vec(file,vec):
  file.write(" %s %s %s "%(swytrailzro(vec[0]), swytrailzro(vec[1]),swytrailzro(vec[2])) )
  
def save_ground_specs():
  file = open(export_dir + "/Data/ground_specs.txt","w")
  for ground_spec in ground_specs:
    file.write(" %s %d %s %s %s"%(ground_spec[0],ground_spec[1],ground_spec[2],swytrailzro(ground_spec[3]),ground_spec[4]))
    if (ground_spec[1] & gtf_has_color):
      file.write(" %s %s %s"%(swytrailzro(ground_spec[5][0]), swytrailzro(ground_spec[5][1]), swytrailzro(ground_spec[5][2]) ))
    file.write("\n")
  file.close()
