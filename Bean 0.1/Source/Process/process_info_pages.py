...
def save_info_pages():
  ...
  for i_info_page in xrange(len(info_pages)):
    ...
    #ofile.write("%s %s %s"%(signature[i_info_page%sign_length],string.replace(info_page[1]," ","_"), string.replace(info_page[2]," ","_")))
    ofile.write("ip_%s %s %s"%(str(i_info_page),string.replace(info_page[1]," ","_"), string.replace(info_page[2]," ","_")))
    ofile.write("\n")
  ofile.close()
