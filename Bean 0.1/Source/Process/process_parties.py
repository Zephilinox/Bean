...
#    file.write(" 1 %d "%(i_party))
    ...
    #file.write("%s %s %d "%(signature[(find_object(parties,convert_to_identifier(party[0])))%sign_length],replace_spaces(party[1]),party[2]))
    file.write("p_%s %s %d "%(str(i_party),replace_spaces(party[1]),party[2]))
    menu_no = 0
    menu_param = party[3]
