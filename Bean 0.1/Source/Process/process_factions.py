...
def save_factions(relations):
  ...
  for i_faction in xrange(len(factions)):
    ...
    #file.write("%s %s %d %d \n"%(signature[(find_object(factions,convert_to_identifier(faction[0])))%sign_length], replace_spaces(faction[1]), faction[2], fac_color))
    file.write("fac_%s %s %d %d \n"%(str(i_faction), replace_spaces(faction[1]), faction[2], fac_color))
