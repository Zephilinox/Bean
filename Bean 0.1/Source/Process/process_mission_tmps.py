...
def save_triggers(file,template_name,triggers,variable_list,variable_uses,tag_uses,quick_strings):
  ...
  for i in xrange(len(triggers)):
    ...
    save_statement_block(file, 0, 1, trigger[trigger_conditions_pos]  , variable_list,variable_uses,tag_uses,quick_strings, "mst_" + template_name + ", trigger no " + str(i) + "'s condition block" )
    save_statement_block(file, 0, 1, trigger[trigger_consequences_pos], variable_list,variable_uses,tag_uses,quick_strings, "mst_" + template_name + ", trigger no " + str(i) + "'s consequence block" )
    file.write("\n")
  file.write("\n")
...
def save_mission_templates(variables,variable_uses,tag_uses,quick_strings):
  ...
  for mission_template in mission_templates:
    ...
    #file.write("%s %s %d "%(signature[i_tmps%sign_length],convert_to_identifier(mission_template[mission_template_name_pos]),mission_template[mission_template_flags_pos]))
    file.write("mst_%s %s %d "%(str(i_tmps),convert_to_identifier(mission_template[mission_template_name_pos]),mission_template[mission_template_flags_pos]))
    i_tmps+=1
