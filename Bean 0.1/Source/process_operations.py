  ...
  if (underscore_pos > 0):
    ...
    if (tag_type > 0):
      if (id_no < 0):
        ...
        # Dunde's Protection BEGIN
        if (tag_type<>tag_string):
          tag_type = 0
        result = id_no | (tag_type << op_num_value_bits)
        # Dunde's Protection END
    else:
...
def get_variable(variable_string,variables_list,variable_uses, calling_script):
  ...
  if not found:
      if variable_string[0] == '$':
         ...
         result = len(variables_list) - 1
         print "WARNING: Usage of unassigned global variable: " + variable_string + " in " + calling_script
      else:
         print "ERROR: Usage of unassigned local variable: " + variable_string + " in " + calling_script
  return result  
...
def save_statement(ofile,opcode,no_variables,statement,variable_list,variable_uses,local_vars_list,local_var_uses,tag_uses,quick_strings, calling_script):
  ...
  for i in xrange(lenstatement):
    ...
    # Ikaguia's error detection BEGIN
    try:
...
def save_statement_block(ofile,statement_name,can_fail_statement,statement_block,variable_list, variable_uses,tag_uses,quick_strings, calling_script):
  ...
  last_is_else_try = 0
  last_is_try_begin = 0
  last_is_troop_get_type = 0
  nested_begins = [try_begin,
                   try_for_range,
                   try_for_range_backwards,
                   try_for_parties,
                   try_for_agents, ]
  if use_wse==1:
    nested_begins += [try_for_attached_parties, # WSE
                      try_for_active_players,   # WSE
                      try_for_prop_instances,   # WSE
                      try_for_dict_keys, ]      # WSE                     
  #############################
  current_depth = 0
  can_fail = 0
  for i in xrange(len(statement_block)):
    ...
    if ((type(statement) != types.ListType) and (type(statement) != types.TupleType)):
      ...
      opcode = statement[0]
      no_variables = 0
    if (opcode in nested_begins):
      current_depth = current_depth + 1
    elif (opcode == try_end):
      current_depth = current_depth - 1
    elif (opcode == else_try) and (last_is_else_try==1):
      print "WARNING : Consecutive (else_try), used at operation #" + str(i) + " in " + calling_script
    elif (opcode == else_try) and (last_is_try_begin==1):
      print "WARNING : Using of (else_try), immediately following (try_begin), at operation #" + str(i) + " in " + calling_script
    elif (opcode <> val_and) and (last_is_troop_get_type==1) and (ignore_troop_get_type==0):
      print "WARNING : Using of troop_get_type without following by val_and at operation #" + str(i) + " in " + calling_script      
    ...
    if (opcode == try_begin):
	  ...
	  last_is_try_begin= 0	
    if (opcode == troop_get_type):
      last_is_troop_get_type= 1
    else:
	  last_is_troop_get_type= 0	
  ...
  while (i < len(local_vars)):
    if (local_var_uses[ i] == 0 and not(local_vars[ i].startswith("unused"))):
      print "WARNING: Local variable never used: :" + local_vars[ i] + " in " + calling_script
    i = i + 1
...
def save_simple_triggers(ofile,triggers,variable_list, variable_uses,tag_uses,quick_strings,additional_strings):
  ofile.write("%d\n"%len(triggers))
  trigger_id = 0
  for trigger in triggers:
    ofile.write("%f "%(trigger[0]))
    save_statement_block(ofile,0,1,trigger[1]  , variable_list, variable_uses,tag_uses,quick_strings, additional_strings + ", trigger no " + str(trigger_id)  )
    ofile.write("\n")
    trigger_id += 1
  ofile.write("\n")